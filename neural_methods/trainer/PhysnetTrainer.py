    ...
    def train(self, data_loader):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                ...
                if torch.isnan(rPPG).any() or torch.isinf(rPPG).any():
                    print("rPPG contains NaN or Inf values")
                if torch.isnan(BVP_label).any() or torch.isinf(BVP_label).any():
                    print("BVP_label contains NaN or Inf values")
    ...
    def intro_valid(self, data_loader):
        ...
        if self.config.TOOLBOX_MODE == "only_test":
            if not os.path.exists(self.config.INFERENCE.MODEL_PATH):
                raise ValueError("Inference model path error! Please check INFERENCE.MODEL_PATH in your yaml.")
            self.model.load_state_dict(torch.load(self.config.INFERENCE.MODEL_PATH))
            print("Validating uses pretrained model!")
            print(self.config.INFERENCE.MODEL_PATH)
